//! AlgoTom GridRec parameters, the test reconstruction of the two selected
//! slices (through algotom's `gridrec_reconstruction` in the
...
/// The interpreter of the pixi environment that has algotom (and tomopy,
/// whose gridrec it wraps) installed.
pub const ALGOTOM_PYTHON: &str =
...
pub const CONFIG_NAME: &str = "algotom_gridrec_config";

/// Rows of one test band around a selected line: gridrec reconstructs each
/// sinogram row independently, so a single row per line is shipped (the
/// band is `BAND × rebin` rows tall when the test data is rebinned first).
...
/// The gridrec filters (tomopy's set; "none" = no filter).
...
/// The AlgoTom GridRec parameters used by the pipeline's
/// `gridrec_reconstruction` call, with its defaults.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct GridrecParams {
    /// Filter (pipeline default: shepp); index into `FILTERS`.
    pub filter: usize,
    /// Radius ratio of the circle mask applied to the reconstruction (1.0).
    pub ratio: f64,
    /// FFT edge padding in pixels (pipeline default: 100).
    pub pad: i64,
    /// Filter parameter (butterworth cutoff, algotom default 0.9).
    pub filter_par: f64,
    /// Center of rotation in pixels (column of the sinogram).
    pub center: f64,
}

impl Default for GridrecParams {
    fn default() -> Self {
        Self {
            filter: 1, // shepp
            ratio: 1.0,
...
impl GridrecParams {
    /// Defaults seeded from the stack: the saved `algotom_gridrec_config`
    /// when the checkpoint carries one, otherwise the stack's center of
    /// rotation (falling back to the middle of the detector).
    ...
    /// The saved form matches the pipeline's `gridrec_reconstruction` call:
    /// `filter_name` is null for "none".
    pub fn to_json(&self) -> String {
        let filter = FILTERS[self.filter.min(FILTERS.len() - 1)];
        serde_json::json!({
            "filter_name": if filter == "none" {
                serde_json::Value::Null
            } else {
                serde_json::Value::from(filter)
            },
            "ratio": self.ratio,
            "pad": self.pad,
    ...
    pub fn from_json(text: &str) -> Option<Self> {
        ...
        let mut params = Self::default();
        match doc.get("filter_name") {
            Some(v) if v.is_null() => params.filter = 0,
            Some(v) => {
                if let Some(i) = v
                    .as_str()
                    .and_then(|name| FILTERS.iter().position(|f| *f == name))
                {
                    params.filter = i;
                }
            }
        ...
        if let Some(v) = doc.get("pad").and_then(|v| v.as_i64()) {
            params.pad = v;
        }
        if let Some(v) = doc.get("filter_par").and_then(|v| v.as_f64()) {
            params.filter_par = v;
        }
    ...
    pub fn describe(&self) -> String {
        let filter = FILTERS[self.filter.min(FILTERS.len() - 1)];
        let mut text = format!(
            "gridrec, {filter} filter, ratio {:.2}, pad {}",
            self.ratio, self.pad
        );
        if filter == "butterworth" {
            text.push_str(&format!(", cutoff {:.2}", self.filter_par));
        }
...
for row in spec["rows"]:
    img = rec.gridrec_reconstruction(
    ...
    slices.append(np.array(np.squeeze(img), dtype=np.float32))
...
fn scratch_dir(stack: &LoadedStack) -> Result<PathBuf, String> {
    ...
    let dir = base.join(format!(".algotom_gridrec_optimizer_{}", std::process::id()));
    if std::fs::create_dir_all(&dir).is_ok() {
        return Ok(dir);
    }
    let dir =
        std::env::temp_dir().join(format!("algotom_gridrec_optimizer_{}", std::process::id()));
...
fn run_recon(
    ...
    // reconstruct faster. The parameters keep the checkpoint's pixel units;
    // only the center of rotation follows the pixel grid of the rebinned
    // width (the same rule as the pre-processing rebin step).
    ...
    // One BAND-row band around each selected line; its middle row is shown.
    let bands = [
    ...
    let script = dir.join("algotom_gridrec_run.py");
    let cleanup = || {
    ...
    let run = || -> Result<(usize, usize, Vec<f32>, Vec<f32>), String> {
        // Gridrec reconstructs each sinogram row independently, so only the
        // two selected rows are shipped (at the test resolution).
        ...
        std::fs::write(&script, ALGOTOM_SCRIPT)
            .map_err(|e| format!("write {}: {e}", script.display()))?;
        let output = std::process::Command::new(ALGOTOM_PYTHON)
            .arg(&script)
            .arg(&sino_npy)
            .arg(&spec_file)
            .arg(&out_npy)
            .output()
            .map_err(|e| format!("cannot launch {ALGOTOM_PYTHON}: {e}"))?;
        ...
        let [count, rh, rw] = shape.as_slice() else {
            return Err(format!("algotom returned shape {shape:?}, expected 3-D"));
        };
        if *count != 2 {
            return Err(format!("algotom returned {count} slices, expected 2"));
        }
...
/// Write (or replace) the `algotom_gridrec_config` JSON in the checkpoint's
/// `/metadata` group, where the main application reads it back.
pub fn save_params(path: &Path, params: &GridrecParams) -> Result<(), String> {
    use hdf5_metno::types::VarLenUnicode;
...
mod tests {
    ...
    fn params_json_roundtrip() {
        ...
        let back = GridrecParams::from_json(&params.to_json()).unwrap();
        assert_eq!(back, params);
        let doc: serde_json::Value = serde_json::from_str(&params.to_json()).unwrap();
        assert_eq!(doc["filter_name"], "butterworth");
        // The "none" filter is saved as null.
        let none = GridrecParams { filter: 0, ..params };
        let doc: serde_json::Value = serde_json::from_str(&none.to_json()).unwrap();
        assert!(doc["filter_name"].is_null());
        assert_eq!(GridrecParams::from_json(&none.to_json()).unwrap().filter, 0);
        assert!(GridrecParams::from_json("nope").is_none());
    }
    ...
    fn band_rows_hold_band_slices_after_rebin() {
        // Full resolution: the selected row itself.
        assert_eq!(band_rows(100, 2048, 1), (100, 101));
        // 2x2: 2 rows around row 100 -> rebinned to 1 slice.
    ...
    fn extract_bands_rebins_each_band() {
        ...
        let full = extract_bands(&p, [(1, 2), (6, 7)], 1);
        assert_eq!(full, vec![1.0, 1.0, 1.0, 1.0, 6.0, 6.0, 6.0, 6.0]);
        // 2x2: rows (0,1) -> 0.5, rows (6,7) -> 6.5; 2 px wide.
        let reb = extract_bands(&p, [(0, 2), (6, 8)], 2);
        assert_eq!(reb, vec![0.5, 0.5, 6.5, 6.5]);
