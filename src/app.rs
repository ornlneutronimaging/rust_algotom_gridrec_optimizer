...
//! AlgoTom GridRec reconstruction of those two slices, tune the parameters,
//! repeat — then save the parameters into the checkpoint HDF5.

use crate::recon::{
    CONFIG_NAME, FILTERS, GridrecParams, ReconJob, TEST_REBIN_FACTORS, checkpoint_geometry,
    max_test_rebin, save_params, tilt_tool_records,
};
...
/// it carries none (the same rule `GridrecParams::from_stack` seeds from).
...
/// applications) and the official Algotom logo, both embedded in the binary
/// and shown at the right end of the toolbar.
const IMAGING_LOGO_BYTES: &[u8] = include_bytes!("../logos/ImagingLogo.png");
const ALGOTOM_LOGO_BYTES: &[u8] = include_bytes!("../logos/algotom_logo.png");
...
pub struct OptimizerApp {
    ...
    /// Set while reloading after the tool changed the file: seed the center
    /// of rotation from the file's new value instead of the (now stale)
    /// saved parameters.
...
impl OptimizerApp {
    ...
    fn adopt_stack(&mut self, stack: LoadedStack) {
        ...
        self.params = GridrecParams::from_stack(&stack);
        let restored = stack
            .metadata
            .iter()
            .any(|(name, _)| name == CONFIG_NAME);
        let mut note = if restored {
            " — saved AlgoTom GridRec parameters restored"
        } else {
            ""
        }
        .to_owned();
        if std::mem::take(&mut self.reseed_center_on_load) {
            if let Some(cor) = stack.center_of_rotation {
                ...
                note = format!(
                    " — center of rotation re-seeded from the corrected file \
                     ({cor:.2} px); save the parameters to keep it"
                );
            }
        } else {
            // Default test data: the checkpoint's own resolution (gridrec is
            // fast; the rebin is only a shortcut).
            self.test_rebin = 1;
        }
        self.status = format!(
            "{} — {} projections{note}",
            ...
            egui::ComboBox::from_id_salt("test_rebin")
                ...
                .on_hover_text(
                    "reconstruct the test slices from n×n block-averaged projections: \
                     ...
                     checkpoint as is (only the center of rotation is converted for \
                     the rebinned test run).",
                );
        });
        ui.label(
            RichText::new(format!(
                "the test reconstruction runs on the two marked slices only (gridrec \
    ...
    fn center_readout(&self, ui: &mut egui::Ui, stack: &LoadedStack, w: usize) {
        ...
        ui.label(
            RichText::new(format!(
                "{:+.2} px from the detector center ({:.1} px) — adjustable in the \
                 Advanced section",
                current_cor - w as f64 / 2.0,
                w as f64 / 2.0,
            ))
            .weak()
            .size(11.0),
    ...
    fn params_panel(&mut self, ui: &mut egui::Ui) {
        ui.label(RichText::new("AlgoTom GridRec parameters").strong());
        ...
        egui::CollapsingHeader::new(RichText::new("🔒 Advanced").strong())
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add(egui::DragValue::new(&mut self.params.pad).range(0..=1000))
                        .on_hover_text(
                            "padding that reduces the FFT edge artifacts; the pipeline \
                             uses 100",
                        );
                });
                ui.add(
                    egui::Slider::new(&mut self.params.filter_par, 0.0..=1.0)
                        .step_by(0.05)
                        .text("filter parameter"),
                )
                .on_hover_text("cutoff of the butterworth filter; ignored by the others");
                let width = self
                    .stack
                    .as_ref()
                ...
                let file_cor = self
                    .stack
                    .as_ref()
                    .and_then(|s| s.sample.first().map(|p| checkpoint_center(s, p.width).0))
                    .unwrap_or(width / 2.0);
                ui.horizontal(|ui| {
                    ...
                    if (self.params.center - file_cor).abs() > 1e-6 {
                        ui.label(
                            RichText::new(format!(
                                "moved from {file_cor:.2} px in the checkpoint ({:+.2})",
    ...
    fn results_panel(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
                ...
                None => {
                    ui.horizontal(|ui| {
                        ...
                        ui.label(if self.result_rebin > 1 {
                            format!(
                                "gridrec is reconstructing the two test slices ({0}x{0} \
                                 rebinned)…",
                                self.result_rebin
                            )
                        } else {
                            "gridrec is reconstructing the two test slices…".to_owned()
                        });
                    });
        ...
        ui.horizontal(|ui| {
                ...
                .clicked()
            {
                ...
                self.recon_error = None;
                self.status = "Running gridrec…".to_owned();
            ...
            ui.horizontal(|ui| {
                ...
                let logos = self.logo_tex.get_or_insert_with(|| {
                    [
                        ...
                        ("algotom_logo", ALGOTOM_LOGO_BYTES),
                    ]
                    .into_iter()
            ...
            egui::Panel::bottom("actions").show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui
                        ...
                        .on_hover_text(
                            "writes algotom_gridrec_config into the checkpoint so later \
