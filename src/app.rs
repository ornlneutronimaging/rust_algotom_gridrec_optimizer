...
use crate::recon::{CONFIG_NAME, FILTERS, ReconJob, GridrecParams, save_params};
    ...
    fn results_panel(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        if let Some(job) = &mut self.recon_job {
            match job.poll() {
                ...
                None => {
                    ui.horizontal(|ui| {
                        ui.spinner();
                        ui.label("gridrec is reconstructing the two test slices…");
                    });
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                ...
                .on_hover_text(
                    "writes algotom_gridrec_config into the checkpoint so later reconstructions \
