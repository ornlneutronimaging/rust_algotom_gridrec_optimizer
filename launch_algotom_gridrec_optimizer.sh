#!/usr/bin/env bash
# Launch the AlgoTom Optimizer GUI, rebuilding first if the sources changed.
#
# Usage: ./launch_algotom_gridrec_optimizer.sh [checkpoint.h5]
set -euo pipefail

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BINARY="$REPO_DIR/target/release/algotom_gridrec_optimizer"
...
if $needs_build; then
    ...
    echo "Building algotom_gridrec_optimizer (release)..."
